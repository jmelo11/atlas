...
TEST(TestZeroRateCurveNodeConsistency, TestCurves) {
	...
	atlas::Date refDate(1, atlas::Month::Apr, 2022);
	for (int i = 0; i < 10; i++)
	{
		times.push_back(i);
	...
	atlas::ZeroRateCurve<atlas::Linear> curve(refDate, times, rates);
	for (int i = 1; i < 10; i++)
	{
...
TEST(TestDiscountCurveNodeConsistency, TestCurves) {
	...
	atlas::DiscountCurve<atlas::Linear> curve(refDate, times, dfs);
	for (int i = 1; i < 10; i++)
	{		
...
TEST(TestDiscountSorted, TestCurves) {
	...
	atlas::ZeroRateCurve<atlas::Linear> curve(refDate, shuffleTimes, shuffleRates);
	for (int i = 1; i < 10; i++)
	{
		EXPECT_FLOAT_EQ(curve.forwardRate(0, times[i]), rates[i]);
		EXPECT_FLOAT_EQ(curve.df(i), dfs[i]);
...
TEST(TestZeroRateSorted, TestCurves) {
	...
	atlas::DiscountCurve<atlas::Linear> curve(refDate, shuffleTimes, shuffleDFS);
	for (int i = 1; i < 10; i++)
	{
		EXPECT_FLOAT_EQ(curve.forwardRate(0, times[i]), rates[i]);
		EXPECT_FLOAT_EQ(curve.df(i), dfs[i]);
