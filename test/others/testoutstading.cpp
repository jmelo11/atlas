...
TEST(CalculateOutstandingTest, AdditionalDates) {
    std::map<Date, double> redemptions   = {{Date(27, Month::Aug, 2023), 50}, {Date(27, Month::Sep, 2023), 50}, {Date(27, Month::Nov, 2023), 50}};
    std::map<Date, double> disbursements = {{Date(27, Month::Jun, 2023), 150}};
    std::set<Date> additional            = {Date(27, Month::Oct, 2023)};

    std::vector<std::tuple<Date, Date, double>> result = calculateOutstanding(disbursements, redemptions, additional);
    ...
    EXPECT_EQ(Date(27, Month::Oct, 2023), std::get<0>(result[3]));
    EXPECT_EQ(Date(27, Month::Nov, 2023), std::get<1>(result[3]));
    EXPECT_DOUBLE_EQ(50, std::get<2>(result[3]));
}