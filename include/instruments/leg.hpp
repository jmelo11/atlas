...
namespace atlas {
	...
	struct MetaData {
		MetaData() {};
		MetaData(const std::string& discountCurve, const RateIndex& index):
			discountCurve_(discountCurve), index_(index) {}
	...
	public:
		...
		Leg(const std::vector<Date>& dates, const std::vector<double>& redemptions):
			dates_(dates), redemptions_(redemptions) {
			if (dates.size() - 1 != redemptions.size())
				throw std::runtime_error("Leg: dates must have remdemptions size+1");
		...
		const auto& constDFIdx() const {
			return DFIdx_;
		}
		
		const auto& constFWDIdx() const {
			return FWDIdx_;
		}
		
		const auto& constRedemptions() const {
			return redemptions_;
		}
		
		const auto& constOutstanding() const {
			return outstanding_;
		}

		const auto& constInterest() const {
			return interest_;
		}
