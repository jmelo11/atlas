...
#ifndef D2DF2DC9_49A8_4FFF_A70D_965D58D5F9FF
#define D2DF2DC9_49A8_4FFF_A70D_965D58D5F9FF
...
namespace Atlas {
    ...
    class ExternalForwardHW : public Model {
        ExternalForwardHW(const MarketRequest& marketRequest, const Date& refDate, const std::vector<std::vector<double>>& shortRates,
                          const std::unique_ptr<YieldCurve>& curve, double dt = 1 / 360, double alpha = 0.01, double sigma = 0.01)
        : Model(marketRequest),
          refDate_(refDate),
          alpha_(alpha),
          sigma_(sigma),
          shortRates_(shortRates),
          curve_(curve),
          dt_(dt),
          nSim_(shortRates[0].size()){};
       ...
       private:
        ...
        double modelZC(double t, double T, double r) const { return A(t, T) * std::exp(-B(t, T) * r); }
        ...
        double A(double t, double T) const {
            ...
            double value   = B(t, T) * -0.25 * tmp * tmp * B(0.0, 2.0 * t);
            return curve_->df(T) / curve_->df(t) * std::exp(value);
        }

        double B(double t, double T) const { return (1 - std::exp(-alpha_ * (T - t))) / alpha_; }
...
#endif /* D2DF2DC9_49A8_4FFF_A70D_965D58D5F9FF */
