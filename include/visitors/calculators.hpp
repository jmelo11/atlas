...
namespace atlas {
	...
	private:
		...
		void calculateInterests(Leg& leg, const InterestRate& rate) {
			...
			interest.clear();
			if (notionals.empty() || notionals.size() != periods - 1)
				leg.calculateOutstanding();
			for (size_t i = 0; i < periods; i++)
				interest.push_back((rate.compoundFactor(dates.at(i),
					dates.at(i + 1)) - 1) * notionals.at(i));
		}

		void forecast(Leg& leg, double spread) {
			if (leg.outstanding().empty())
				leg.calculateOutstanding();
			const auto& notionals = leg.outstanding();
			const auto& dates = leg.dates();
	...
	public:
		...
		void visit(const DAP& inst) const override {
			npv_ += npv(inst.constLeg());
		}

		void visit(const EqualPayment& inst) const override {
			npv_ += npv(inst.constLeg());
		}

		void visit(const FixedBullet& inst) const override {
			npv_ += npv(inst.constLeg());
		}

		void visit(const FloatingBullet& inst) const override {
			npv_ += npv(inst.constLeg());
		}

		void visit(const FloatingCustomStructure& inst) const override {
			npv_ += npv(inst.constLeg());
		}

		void visit(const FixedCustomStructure& inst) const override {
			npv_ += npv(inst.constLeg());
		}

		void visit(const VanillaSwap& inst) const override {
			int flag = inst.side() == VanillaSwap::Side::Receiver ? 1 : -1;
	...
	public:
		...
		void visit(const DAP& inst) const override {
			npv_ += npvAndForecast(inst.constLeg(), inst.rate());
		...
		void visit(const FloatingBullet& inst) const override {
			npv_ += npvAndForecast(inst.constLeg(), inst.spread());
	...
	public:
		ParRateSolver(const MarketData& marketData, double accuracy = 0.0001, double guess = 0.01, double step = 0.0001)
			...
			if (marketData_.dfs.empty())
				throw std::runtime_error("MarketData does not contain any discount factors");			
		}

		/* podria ser más eficiente al no ocupar el solver */
		void visit(DAP& inst) override {
