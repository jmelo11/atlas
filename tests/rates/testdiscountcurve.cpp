...
TEST(DiscountCurve, BasicTest) {
    ...
    DiscountCurveStrategy<double, LinearInterpolator<double>> curve(dates, discounts, dayCounter);
...
TEST(DiscountCurve, DiscountFactorTest) {
    ...
    std::vector<double> discounts = {1, 0.9, 0.85};
    DayCounter dayCounter         = Actual360();
    DiscountCurveStrategy<double, LogLinearInterpolator<double>> curve(dates, discounts, dayCounter);
...
TEST(DiscountCurve, InvalidArgumentsTest) {
    ...
    auto f = [&]() { DiscountCurveStrategy<double, LinearInterpolator<double>> curve(std::vector<Date>{}, discounts, dayCounter); };
    EXPECT_THROW(f(), std::invalid_argument);

    auto g = [&]() { DiscountCurveStrategy<double, LinearInterpolator<double>> curve(dates, std::vector<double>{}, dayCounter); };
    EXPECT_THROW(g(), std::invalid_argument);

    auto h = [&]() { DiscountCurveStrategy<double, LinearInterpolator<double>> curve(dates, std::vector<double>{1}, dayCounter); };
